...
const sum = (re) => { const u = [...new Set(html.match(re) || [])]; let gz = 0; for (const x of u) gz += zlib.gzipSync(fs.readFileSync('.next' + x.replace('/_next', ''))).length; return { n: u.length, gz: Math.round(gz / 1024) } }
