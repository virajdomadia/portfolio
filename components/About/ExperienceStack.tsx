    ...
    <div className={s.exp} data-progress="span">
      {content.experience.map((e, i) => (
        <article key={e.title} className={s.card} style={{ ['--i' as string]: i }} data-reveal>
          ...
          <div>
            <b>{e.orgUrl ? <a href={e.orgUrl} target="_blank" rel="noreferrer">{e.title}</a> : e.title}</b>
            <p>{e.desc}</p>
            {e.extra && <p className={s.extra}>{e.extra}</p>}
            {e.chips && <div className={s.chips}>{e.chips.map((c) => <span key={c}>{c}</span>)}</div>}
          </div>
        </article>
      ))}
    </div>
