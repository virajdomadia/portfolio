  ...
  return (
    <section className={s.band} data-progress="band" aria-label={headline}>
      ...
      <div className={`wrap ${s.txt}`}>
        <p className={s.big}><span className={s.srOnly}>{headline}</span>{[...headline].map((c, i) => c === ' ' ? ' ' : <span key={i} className={s.ch} data-ch style={{ ['--i' as string]: i }} aria-hidden="true">{c}</span>)}</p>
